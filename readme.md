...
영상 이진화는 컬러 혹은 gray scale로 이루어진 이미지를 0(검정), 1(흰색)(혹은 255)으로 변환시키는 것을 의미합니다.
...
어두운 픽셀들의 비율을 α, 밝기 평균을 μ1, 분산을 σ1^2, T보다 밝은 픽셀들의 비율을 β, 밝기 평균을 μ2, 분산을 σ2^2라 했을 때 (α+β=1), intra-class 분산과 inter-class 분산은 각각 다음과 같습니다.
